...
pub(crate) struct Worker {
  pub(crate) event_sender: UnboundedSender<Event>,
  loader: Loader,
...
mod tests {
  ...
  async fn errors_when_tool_call_limit_is_exceeded() {
    ...
    let test_worker = TestWorker::new(tool_calls, true);

    let error = test_worker
      .worker
      .stream(
        0,
        vec![Message::User(vec![UserMessageContent::Text("foo".into())])],
    ...
    assert!(test_worker.events.is_empty());
  ...
  async fn errors_when_tool_round_limit_is_exceeded() {
    ...
    let mut test_worker = TestWorker::new(tool_calls, true);
    ...
    assert_eq!(
      test_worker.requests.lock().unwrap().len(),
      Worker::MAX_TOOL_ROUNDS + 1
    );

    let mut tool_call_event_count = 0;

    while let Ok(event) = test_worker.events.try_recv() {
      if matches!(
        event,
        Event::Agent {
  ...
  async fn loops_until_provider_returns_no_tool_calls() {
    let mut test_worker = TestWorker::new(
      vec![vec![Output::ToolCall], vec![Output::Delta("done")]],
      true,
    );
    ...
    let requests = test_worker.requests.lock().unwrap();
  ...
  fn system_prompt() {
    ...
    {
      let test_worker = TestWorker::new(Vec::new(), true);

      let agents_path = test_worker.directory.path().join("AGENTS.md");
      ...
      assert_eq!(
        test_worker.worker.system_prompt().unwrap(),
        expected(test_worker.directory.path(), &agents_path)
      );
    }
